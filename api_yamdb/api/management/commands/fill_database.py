...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with open(os.path.join(settings.BASE_DIR, 'static/data/users.csv'),
            ...
            for row in reader:
                if row[0] != 'id':
                    User.objects.get_or_create(id=row[0], username=row[1],
        ...
        with open(os.path.join(settings.BASE_DIR, 'static/data/titles.csv'),
            ...
            for row in reader:
                if row[0] != 'id':
                    Title.objects.get_or_create(id=row[0], name=row[1],
        ...
        with open(os.path.join(settings.BASE_DIR, 'static/data/genre_title.csv'),
            ...
            for row in reader:
                if row[0] != 'id':
                    GenreToTitle.objects.get_or_create(id=row[0], title_id=row[1], genre_id=row[2])
        ...
        with open(os.path.join(settings.BASE_DIR, 'static/data/review.csv'),
            ...
            for row in reader:
                if row[0] != 'id':
                    Review.objects.get_or_create(id=row[0], title_id=row[1],
        ...
        with open(os.path.join(settings.BASE_DIR, 'static/data/comments.csv'),
            ...
            for row in reader:
                if row[0] != 'id':
                    Comment.objects.get_or_create(id=row[0], review_id=row[1],
                                                  text=row[2], author_id=row[3],
                                                  pub_date=row[4])
